...
async function init() {
  ...
  if (collapseCommentsEnabled) {
    collapseAllComments();
    // Also observe for new comments being added
...
async function applyTheme(theme, customPalette, darkMode = false) {
  ...
  // Always clear inline styles when switching themes for a clean slate
  clearSvgInlineStyles();
...
// Clear inline styles from vote buttons and SVGs (for clean theme switching)
function clearSvgInlineStyles() {
  // Clear SVG and path inline styles
  const svgSelectors = [
  ...
  // Clear button inline styles too (in case any were applied)
