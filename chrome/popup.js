...
async function init() {
  ...
  document.getElementById('collapseComments').addEventListener('change', async (e) => {
    ...
    await chrome.storage.local.set({ collapseComments });
  ...
  document.getElementById('darkModeToggle').addEventListener('click', async () => {
    ...
    await chrome.storage.local.set({ darkMode: globalDarkMode });
    updateDarkModeToggle();

    // Re-apply current theme with new dark mode setting
    const currentResult = await chrome.storage.local.get(['theme', 'customPalette']);
...
async function saveCustomPalette(palette) {
  await chrome.storage.local.set({ customPalette: palette });
}
...
async function notifyContentScript(theme, customPalette, darkMode) {
  try {
    ...
    if (tabs[0]?.url?.includes('digg.com')) {
      await chrome.tabs.sendMessage(tabs[0].id, { type: 'themeChanged', theme, customPalette, darkMode });
    }
  } catch (e) {
...
async function notifyCollapseChange(collapseComments) {
  try {
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tabs[0]?.url?.includes('digg.com')) {
      await chrome.tabs.sendMessage(tabs[0].id, { type: 'collapseChanged', collapseComments });
    }
  } catch (e) {
    // Content script might not be loaded
    console.log('Could not notify content script:', e);
